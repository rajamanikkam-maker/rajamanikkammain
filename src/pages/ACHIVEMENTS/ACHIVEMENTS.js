import React from "react";
import "./ACHIVEMENTS.css";
import Spin from "react-reveal/Spin";
const ACHIVEMENTS = () => {
  return (
    <>
      <div className="continer project" id="projects">
        <h2 className="col-12 mt-3 mb-1 text-center text-uppercase">
          ACHIVEMENTS
        </h2>
        <hr />
        <p className="pb-3 text-center">THESE ARE ALL MY ACHIVEMENTS
        
        </p>
        {/* card design */}
        <div className="row" id="ads">
          <Spin>
            <div className="col-md-4">
              <div className="card rounded">
                <div className="card-image">
                  <span className="card-notify-badge">CLOUD</span>
                  <img
                    src="https://mite.ac.in/wp-content/uploads/2020/01/nptel-mite.jpg"
                    alt="project1"
                    rel="noreferrer"
                    target="_blank"
                  />
                </div>
                <div className="card-image-overly m-auto mt-3">
                  <span className="card-detail-badge">CLOUD COMPUTING </span>

                  
                </div>
              
                <div className="card-body text-center">
                  
                  <a
                    className="ad-btn"
                    href="https://drive.google.com/file/d/1D9mhLVB-g32rPpNVwrxVaZfYTBd5vKgC/view?usp=drive_link"
                    rel="noreferrer"
                    target="_blank"
                  >
                    View
                  </a>
                </div>
              </div>
            </div>
            <div className="col-md-4">
              <div className="card rounded">
                <div className="card-image">
                  <span className="card-notify-badge">Full Stack.MERN</span>
                  <img
                    src="https://www.drupal.org/files/infosys-logo-PNG.png"
                    alt="project2"
                  />
                </div>
                <div className="card-image-overly m-auto mt-3">
                  <span className="card-detail-badge">FULL STACK .MERN </span>

                  
                </div>
                <div className="card-body text-center">
                  
                  <a
                    className="ad-btn"
                    href="https://drive.google.com/file/d/1OWpZHQAnZezY9OyJYY3WHGDy78WKx73s/view?usp=drive_link"
                    rel="noreferrer"
                    target="_blank"
                  >
                    View
                  </a>
                </div>
              </div>
            </div>
            <div className="col-md-4">
              <div className="card rounded">
                <div className="card-image">
                  <span className="card-notify-badge">PYHTON</span>
                  <img
                    src="data:image/png;base64,"
                    alt="project1"
                  />
                </div>
                <div className="card-image-overly m-auto mt-3">
                  <span className="card-detail-badge">FULL STACK PYTHON</span>
                  
                </div>
                <div className="card-body text-center">
                <a
                    className="ad-btn"
                    href="https://drive.google.com/file/d/15vB76XwwkbCfniQEdLtkvtGBGu0VbhKK/view?usp=drive_link"
                    rel="noreferrer"
                    target="_blank"
                  >
                    View
                  </a>
                 
                  
                </div>
              </div>
            </div>
          </Spin>
        </div>
      </div>
    </>
  );
};

export default ACHIVEMENTS;
